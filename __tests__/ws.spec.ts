...
import { WsClient, WsListener } from "../src/index";
...
describe("WS", () => {
    test("global method call", async () => {
        let closed = false;
        // set up a server
        const listener = new WsListener(api.$specSpace, {
            port: 1234,
        }, (session) => {
        ...
        // create client
        const client = api.$bind(new WsClient(api.$specSpace, "ws://localhost:1234"));
